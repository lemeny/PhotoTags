...
def get_tags_by_photo(conn: sqlite3.Connection) -> dict[int, list[str]]:
    photo_tags = conn.execute(
        """
...
def build_sync_payload(conn: sqlite3.Connection) -> dict:
    photos = conn.execute("SELECT id, filename, original_name, file_sha256 FROM photos").fetchall()
    tags_by_photo = get_tags_by_photo(conn)
    payload_photos = []
    for photo in photos:
...
def index():
    ...
    with get_db_connection() as conn:
        ...
        if selected_tags:
            ...
            photos = conn.execute(
                "SELECT id, filename, original_name FROM photos ORDER BY created_at DESC"
            ).fetchall()
